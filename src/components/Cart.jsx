  ...
  return (
    <li className='flex gap-2 border-gray-300 pb-2 border-b-2'>
      ...
      <section className='flex gap-2 items-center'>
        ...
        {quantity}
        <button onClick={addToCart} className='px-2 bg-sky-900 rounded'>+</button>
      </section>
    </li>
  )
}
...
export function Cart () {
  ...
  return (
    <section>
      <label
        className='absolute bg-sky-400 rounded-full p-2 top-3 right-4 z-10'
      ...
      <aside className='peer-checked:flex flex-col gap-7 rounded-l-2xl hidden w-[320px] bg-sky-700 h-full px-2 absolute top-0 right-0'>
        ...
        <footer className='self-center'>
          <button
            ...
            className='flex justify-center gap-2 w-[150px] bg-red-500 font-amulya p-2 rounded-2xl'
          >
            <span>
              <ClearCart />
            </span>{' '}
